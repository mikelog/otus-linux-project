...
def main():
    ...
    try:
        ...
        #3 if float(proxmox.version.get()['version'].split('-',1)[0]) < 4.0 else 4
    except Exception as e:
    ...
    if state == 'present':
        try:
            ...
            if update:
                ...
                module.exit_json(changed=True, msg="VM %s with newid %s cloned from vm with vmid %s" % (name, newid, vmid), vmid="%s" %newid)
            else:
